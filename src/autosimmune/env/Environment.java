...
public abstract class Environment extends DefaultContext<Agent> {
	 ...
	 * @param citokine Nome da Citocina
	...
	/**
	 * Adiciona um agente ao contexto. O contexto, atraves das classes herdadas de GridAdder presentes em cada Zona,
	 * definirá para qual zona o agente deverá ir, através do atributo "Zone" de cada agente.
	 * @param a Agente a ser adicionado ao contexto
	 */
	 ...
	 * @param a O agente central
	 * @param r Raio
	 * @param type O tipo de agente procurado 
	 * @return Um vetor do tipo "type", contendo os objetos encontrados
	 */
	@SuppressWarnings("unchecked")
	public <T extends Agent> ArrayList<T> getEspecificNeighbors(Agent a, int r, Class<T> type){
	 ...
	 * @param c Agente central, para o qual serao listados os vizinhos
	 * @return Uma lista de agentes
	 */
	 ...
	 * @param c Agente central, para o qual serao listados os vizinhos
	 * @param r Raio: numero de camadas de celulas da grid que serao verificadas
	 * @return Uma lista de agentes
	 */
	...
	/**
	 * Cria uma camada que representa a difusao de uma citocina.
	 * Deve ser chamado no construtor de cada zona
	 * @param citokine Nome da Citocina
	 */
	public void createCitokineLayer(CitokineNames citokine){
		if(!citokines.containsKey(citokine)){
			//cria o grid de valores
			GridValueLayer gvl = new GridValueLayer(this.name.toString() + citokine.toString(), true, new WrapAroundBorders(), getWidth(), getHeight());

			//cria o layer que utiliza o grid de valores
			ValueLayerDiffuser vld = new ValueLayerDiffuser(gvl, global.getFloatParameter(EnvParameters.EVAPORATION_RATE), global.getFloatParameter(EnvParameters.DIFFUSION_CONSTANT), true);
			...
			System.err.println("Tentou criar citocina que ja existe: " + citokine.name());
		}
	}
	...
	public void releaseCitokine(CitokineNames citokine, int x, int y){
		releaseCitokine(citokine, global.getFloatParameter(EnvParameters.CITOKINE_VALUE), x, y);
	}
	 ...
	 */
	public void releaseCitokine(CitokineNames citokine, double value, int x, int y){
		...
		citokines.get(citokine).getValueLayer().get(currentValue + value, x, y);
	}
	 ...
	 * @param c Agente
	 * @param x Distancia a andar em X (celulas da grid)
	 * @param y Distancia a andar em Y (celulas da grid)
	 */
