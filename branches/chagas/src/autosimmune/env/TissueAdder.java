...
public class TissueAdder<T extends Agent> implements GridAdder<T> {
	@Override
	public void add(Grid<T> space, T obj) {	
		if(obj.getZoneName().name().equals(ZoneNames.Tissue.name())){
			if(!space.moveTo(obj, obj.getX(), obj.getY())){
				System.err.println("Erro ao adicionar objeto a zona Tecido.");
			} else {
